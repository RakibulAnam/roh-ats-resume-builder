    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h3 className="text-lg font-semibold text-gray-800 flex items-center gap-2"><Users size={20} /> Affiliations</h3>
                {!isEditing && (
            ...
            <div className="space-y-3">
                {items.length === 0 && !isEditing && <p className="text-center text-gray-400 py-4 text-sm">No affiliations added yet.</p>}
                {items.map(item => (
                    <div key={item.id} className="bg-white border border-gray-100 p-4 rounded-xl hover:shadow-sm transition-shadow group relative">
                        <div className="flex justify-between items-start">
                            <div>
                                <h4 className="font-bold text-gray-900">{item.organization}</h4>
                                <div className="text-indigo-600 font-medium text-sm">{item.role}</div>
                                <div className="text-gray-400 text-xs mt-1">{item.startDate} - {item.endDate}</div>
                            </div>
