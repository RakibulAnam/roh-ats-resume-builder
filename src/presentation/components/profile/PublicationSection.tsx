...
import { Publication } from '../../../domain/entities/Resume';
...
export const PublicationSection = ({ items, onRefresh }: Props) => {
    const { user } = useAuth();
    ...
    const resetForm = () => {
        setFormData({ title: '', publisher: '', date: '', link: '' });
        setEditingId(null);
        setIsEditing(false);
    };
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            await profileRepository.deletePublication(id);
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        setSaving(true);
        try {
            await profileRepository.savePublication(user.id, {
                id: editingId || '',
            ...
            resetForm();
            onRefresh();
        } catch (error) { toast.error('Failed to save publication'); } finally { setSaving(false); }
    };
